...
## 9. 版本控制与自动文档化 (Auto-Documentation)

系统必须保证“代码变更”与“认知总结”的强一致性：

1. **先结后存**：所有提交前必须自动总结当前任务对 `CHANGELOG.md` 和 `README.md` 的影响。

2. **知识沉淀**：总结必须包含：**解决了什么业务痛点**、**逻辑有何重大变化**。

3. **一键同步**：通过 `git_save` 实现总结、提交、推送的原子操作。
